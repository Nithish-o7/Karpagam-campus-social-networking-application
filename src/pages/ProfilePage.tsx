...
export default function ProfilePage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const [allPosts, allTickets] = await Promise.all([
        postService.getFeed(),
        ticketService.getUserTickets(),
      ]);

      // Filter locally for "My" items
      // For tickets, we match by ticket.name (though usually it would be by email/sys_id)
      // For posts, we match by author_name
      setPosts(allPosts.filter(p => p.author_name === user.name));
      setTickets(allTickets.filter(t => t.name === user.name));
    } catch (err) {
    ...
    <PageTransition>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 20, marginBottom: 40, animation: 'fade-up 0.6s cubic-bezier(0.2, 0.8, 0.2, 1)' }}>
        ...
        <div style={{ textAlign: 'center' }}>
          ...
          <div style={{ marginTop: 10, fontSize: 'var(--fs-xs)', color: 'var(--text-muted)', fontWeight: 600, opacity: 0.8 }}>
            {user.email}
          </div>
        </div>
        ...
        <div style={{ 
          ...
          background: 'var(--surface)', padding: '12px 32px', borderRadius: 'var(--r-xl)',
          boxShadow: 'var(--shadow-card)', border: '1px solid var(--border)'
        }}>
          <div style={{ textAlign: 'center', minWidth: 80 }}>
            <div style={{ fontSize: 'var(--fs-lg)', fontWeight: 900, color: 'var(--text-primary)', lineHeight: 1 }}>{posts.length}</div>
            <div style={{ fontSize: '10px', color: 'var(--text-muted)', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.05em', marginTop: 4 }}>Posts</div>
          </div>
          <div style={{ width: 1, height: 24, background: 'var(--border)', alignSelf: 'center', margin: '0 16px' }} />
          <div style={{ textAlign: 'center', minWidth: 80 }}>
            <div style={{ fontSize: 'var(--fs-lg)', fontWeight: 900, color: 'var(--text-primary)', lineHeight: 1 }}>{tickets.length}</div>
            <div style={{ fontSize: '10px', color: 'var(--text-muted)', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.05em', marginTop: 4 }}>Tickets</div>
          </div>
        </div>
      </div>
