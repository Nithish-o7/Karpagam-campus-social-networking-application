          ...
          {events.map((event) => (
            <div key={event.id} style={{ display: 'flex', gap: 16, alignItems: 'center' }}>
              <div style={{ width: 44, height: 44, borderRadius: 14, background: 'var(--bg-app)', border: '1px solid var(--border)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                <span style={{ fontSize: 8, fontWeight: 900, color: 'var(--text-faint)' }}>{event.date.split(',')[0].slice(0,3)}</span>
                <span style={{ fontSize: 15, fontWeight: 900, color: 'var(--crimson)' }}>{event.date.split(' ')[1]}</span>
              </div>
              <div style={{ minWidth: 0 }}>
                ...
                <div style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 2, fontWeight: 500 }}>{event.time} • {event.venue}</div>
              </div>
            </div>
          ))}
        </div>
